...
def build_parser() -> argparse.ArgumentParser:
    ...
    listing.add_argument("--today", type=date.fromisoformat, default=None)
    return parser
...
def main(argv=None) -> int:
    ...
    try:
        if args.entity == "customer" and args.action == "add":
            ...
            as_of = (args.today or date.today()) if args.overdue else None
            for invoice in core.list_invoices(store, args.customer, overdue_as_of=as_of):
                print(json.dumps(_invoice_row(invoice)))
    except ValueError as error:
