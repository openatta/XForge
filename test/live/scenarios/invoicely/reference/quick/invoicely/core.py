...
def is_overdue(invoice: Invoice, today: date) -> bool:
    return invoice.due_on < today and invoice.status != "paid"


def list_invoices(store: Store, customer_id: Optional[str] = None, overdue_as_of: Optional[date] = None) -> List[Invoice]:
    ...
    if overdue_as_of is not None:
        invoices = [i for i in invoices if is_overdue(i, overdue_as_of)]
    return invoices
