...
export type SchemaName = 'manifest' | 'lock' | 'flow' | 'change' | 'agent' | 'gate' | 'rule' | 'hook' | 'script' | 'scaffold' | 'work-package' | 'work-package-delivery';
...
async function buildValidators(): Promise<Map<SchemaName, ValidateFunction>> {
  ...
  addFormats(ajv);
  const names: SchemaName[] = ['manifest', 'lock', 'flow', 'change', 'agent', 'gate', 'rule', 'hook', 'script', 'scaffold', 'work-package', 'work-package-delivery'];
