...
import { OWNERSHIP_PATH, readOwnership } from '../install/ownership.js';
...
interface DoctorData {
   ...
   * Absence is not misconfiguration, so nothing here is ever a warning: made one, a suggestion
   * pushes projects into creating an empty file to silence the tool, and a file that exists but
   * says nothing is worse than none, because it reads as configured.
   */
  suggestions: DoctorFinding[];
}

// Built-in Skills that are never invoked from a Flow Stage by design (chat-driven, standalone).
const STANDALONE_SKILLS = new Set(['xforge-explore', 'xforge-kanban', 'xforge-status', 'xforge-continue', 'xforge-revise', 'xforge-scaffold', 'xforge-archive', 'xforge-upgrade-scaffold']);
