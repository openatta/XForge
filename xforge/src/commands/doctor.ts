...
export async function executeDoctor(project: ProjectContext, options: { kind?: DoctorKind; strict: boolean }): Promise<{
  ...
  /* Mirrors how `approve` resolves a provider id at runtime: `local` is always available for
     interactive approval, an `mcp` provider needs a manifest entry that points at an enabled
     McpServer resource whose command is not an obvious placeholder. A policy backed by no usable
     provider can never actually collect an approval — reported here as an advisory, the same as
     every other doctor finding, so it only escalates when `--strict` is set. */
