...
} satisfies Record<string, { usage: string; description: string; options: string[] }>;

/**
 * The command words, derived from the one table that describes them.
 *
 * Written out three times before this: as a union, as an array the parser validated against, and as
 * the keys of `HELP`. Only the third was checked — `HELP` is a `Record<CommandName, ...>`, so
 * TypeScript already refused an incomplete one — which left the array free to drift from the union
 * with nothing to notice. Deriving both from `HELP` is what makes that impossible rather than
 * merely unlikely; the table below is now the single place a command is declared.
 */
export type CommandName = keyof typeof HELP;
...
/*
 * Explicit, because this order is what `xforge help` lists and that is a surface people read —
 * deriving it from `Object.keys(HELP)` reordered the listing, which is a change to the product
 * dressed as a refactor. What the derivation was for is achieved below instead: a command declared
 * in `HELP` and forgotten here is a compile error that names it, so the two cannot drift while the
 * order stays deliberate.
 */
const COMMANDS = [
  'help', 'version', 'init', 'state', 'install', 'sync', 'update', 'uninstall', 'check', 'stage',
  'advance', 'stage-bundle', 'explain', 'verification', 'transition', 'approve', 'audit',
  'work-package', 'hook', 'archive', 'doctor', 'upgrade-scaffold', 'review', 'findings', 'contract',
] as const satisfies readonly CommandName[];

/** `true` when every command in `HELP` is listed above; otherwise the missing names, as a type error. */
type CommandsListed = Exclude<CommandName, (typeof COMMANDS)[number]> extends never
  ? true
  : ['every command in HELP must be listed in COMMANDS; missing:', Exclude<CommandName, (typeof COMMANDS)[number]>];
const COMMANDS_ARE_COMPLETE: CommandsListed = true;
void COMMANDS_ARE_COMPLETE;
/*
...
export function parseArguments(argv: string[]): ParsedArguments {
  ...
  if (!(COMMANDS as readonly string[]).includes(parsed.command)) {
     ...
     */
    const skillNamed = parsed.command && !(COMMANDS as readonly string[]).includes(parsed.command)
...
export function helpEnvelope(subject?: string, subcommand?: string): Envelope {
  if (subject && !(COMMANDS as readonly string[]).includes(subject)) {
