...
describe('CLI protocol', () => {
  ...
  it('exposes help/version without a project and rejects unknown commands', async () => {
    ...
    expect(version.json.data).toMatchObject({ name: '@xforge/cli', version: '0.8.4', protocolVersion: '2' });
  ...
  it('lets update reconcile an older-but-Protocol-compatible declared CLI version, never install/check/sync', async () => {
    ...
    expect(update.json.changes).toContainEqual(expect.objectContaining({ action: 'modify', path: 'xforge/manifest.yaml', source: 'xforge:declared-version-upgrade:0.7.7->0.8.4' }));
    const manifest = await yamlFile(root, 'xforge/manifest.yaml');
    expect(manifest.xforge.version).toBe('0.8.4');
 ...
 *
 * The instruction shipped in 0.8.4 said `--field governance.revision.contentRevision`. That path
...
describe('documented --field paths resolve', () => {
  ...
  it('resolves every --field example shipped in a Skill, an Agent doc, or the CLI help', async () => {
    ...
    /* The check has teeth only if a wrong path fails, so the exact string that shipped in 0.8.4 is
