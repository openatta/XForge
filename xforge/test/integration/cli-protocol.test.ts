...
describe('CLI protocol', () => {
  ...
  it('exposes help/version without a project and rejects unknown commands', async () => {
    ...
    expect(version.json.data).toMatchObject({ name: '@xforge/cli', version: '0.8.5', protocolVersion: '2' });
 ...
 *
 * The instruction shipped in 0.8.5 said `--field governance.revision.contentRevision`. That path
...
describe('documented --field paths resolve', () => {
  ...
  it('resolves every --field example shipped in a Skill, an Agent doc, or the CLI help', async () => {
    ...
    /* The check has teeth only if a wrong path fails, so the exact string that shipped in 0.8.5 is
