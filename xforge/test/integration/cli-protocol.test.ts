...
describe('CLI protocol', () => {
  ...
  it('exposes help/version without a project and rejects unknown commands', async () => {
    ...
    expect(version.json.data).toMatchObject({ name: '@xforge/cli', version: '0.7.13', protocolVersion: '2' });
  ...
  it('lets update reconcile an older-but-Protocol-compatible declared CLI version, never install/check/sync', async () => {
    ...
    expect(update.json.changes).toContainEqual(expect.objectContaining({ action: 'modify', path: 'xforge/manifest.yaml', source: 'xforge:declared-version-upgrade:0.7.7->0.7.13' }));
    const manifest = await yamlFile(root, 'xforge/manifest.yaml');
    expect(manifest.xforge.version).toBe('0.7.13');
    expect(manifest.scaffold.version).toBe('0.7.13');
    expect(manifest.scaffold.source.version).toBe('0.7.13');
