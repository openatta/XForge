...
1. **Local interactive** — requires a real TTY and `--attestation human`,
   ...
   not automatable: Agents cannot self-approve.
...
3. The platform (or a script polling its API) constructs the receipt JSON
   per the `ApprovalReceipt` schema, computes `governingDigest`, and signs
   it with the shared secret from `approvals.providers[].secretEnv`.
4. Anything — the Agent itself, a CI job, a webhook handler — calls
   `xforge approve --receipt <path>` to import the result.

No XForge code changes are needed for this: MCP is just the transport your
platform happens to use to talk to the Agent; the trust boundary XForge
actually checks is the HMAC signature. The one thing that *does* need code
changes is swapping HMAC-shared-secret verification for a different scheme
entirely (asymmetric signing, a live callback to an external API) —
`manifest.schema.json` currently pins provider `type` to the constant
`"hmac-sha256"`, and `verifyApprovalReceipt()` has no other branch.
