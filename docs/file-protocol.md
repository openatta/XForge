...
> **Implementation status:** implemented by `@xforge/cli 0.7.17` (`xforge.dev/v1alpha2`). Protocol 1 projects remain readable in Portable mode, but Protocol 2 managed writes require an exact CLI/Lock identity.
...
- CLI declaration and Lock: version `0.7.17`, protocol `"2"`, exact runtime integrity.
...
Approval is a first-class receipt bound to Change, Flow, Stage, transition, policy and revision, produced through exactly two mechanisms: local (an interactive terminal running a `readline` dialogue that asks for actor, role, decision and reason live — no confirmation code, and it is repository-level self-attestation, not enterprise-grade identity proof) or an `mcp` provider (a registered `McpServer`, submit + poll). Neither receipt is signed; trust comes from the project's own tamper-evident audit hash chain — a receipt is only valid if a matching `approval.decided` event exists in that chain, and it is revalidated on every state load. Major policies accept both provider kinds, like every other Flow; the shipped `enterprise-approvals` provider is a deliberately non-working placeholder, so until it is replaced Major decisions are in practice captured through `local`. Each Major approval point requires one approver with `separationOfDuties: true`, which means the approver is not an implementer of this Change (the Git authors of the Change directory and of every work-package delivery range) — it does not compare roles.
