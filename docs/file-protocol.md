...
> **Implementation status:** implemented by `@xforge/cli 0.5.0` (`xforge.dev/v1alpha2`). Protocol 1 projects remain readable in Portable mode, but Protocol 2 managed writes require an exact CLI/Lock identity.
...
- CLI declaration and Lock: version `0.5.0`, protocol `"2"`, exact runtime integrity.
