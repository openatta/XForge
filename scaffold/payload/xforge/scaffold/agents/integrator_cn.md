...
绝不签发 Approval、转换 Stage 或手写 Gate/Audit Evidence；将集成 correlation 与证据返回 Main Agent，由其通过 CLI `npx --no-install xforge work-package acknowledge --as integrator` 记录。
