...
绝不签发 Approval、转换 Stage 或手写 Gate/Audit Evidence；将集成 correlation 与证据返回 Main Agent，由其通过 CLI `work-package acknowledge --as integrator` 记录。当所遵循的 Skill 要求直接运行 XForge CLI 时，一律以 `npx --no-install xforge <command> ...` 调用——project-local 安装不在本 shell 的 `PATH` 上。
