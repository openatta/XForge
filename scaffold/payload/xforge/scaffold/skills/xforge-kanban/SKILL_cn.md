...
- 本 Skill 是只读的项目报告能力,独立于 Change/Flow/Gate 状态;不得为它查询 `xforge state`,也不得涉及 `xforge/changes`、`xforge/specs`、Evidence 或 Approval。
- 以 `git log` 作为唯一事实来源。绝不编造脚本产出之外的提交、作者、日期或计数。
