...
2. 每个 `changed` 文件：两份都读。吸收新版**规定**的东西，保住本项目**知道**的东西。一个带着真实测试命令的 Gate、一份本项目在跑的 Script 代码、一段本项目选定的 Skill 措辞、一个有人调过的阈值——那些是关于这个项目的事实，要活过升级。 对 Skill 而言，这条原则有一个可以指认的落点：**`# 判断要点` 是项目的段落，`# 不变量`、`# 权限`、`# 执行` 是产品的。**前者装的是这个项目对「该怎么工作」的决定；后三者装的是 CLI 与 Flow 必须被如何驱动，一个项目保留自己那份，等于让自己的 Agent 被一个已经前进了的控制面拒绝。后三段取上游，判断要点保留自己的，其余两段（`# 证据`、`# 停止与返工`）按内容各自判断。这是一条约定而非工具强制：没有任何东西比较段落，它只在合并者遵守时成立。
