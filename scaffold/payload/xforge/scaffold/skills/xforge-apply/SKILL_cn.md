...
- 从磁盘读取 Action 的全部 inputs、Constitution、相关 Rules/Specs、可选 Design/Check report 和现有工作包；聊天记忆不是事实源。在此之前先运行 `xforge stage-bundle --change <id>`：它指出哪些输入自本 Stage 开始以来变过、哪些没变，于是重读覆盖的是变化的部分而不是全部。只要这个 Change 还有未提交的改动，它就不为任何文件出具凭证。
