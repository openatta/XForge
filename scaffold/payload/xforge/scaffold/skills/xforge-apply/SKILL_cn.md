...
- 开始及每次材料性变更后运行 `xforge state --change <id> --field nextActions --field diagnostics --field change --field context`；只消费当前 revision 的 ready Apply Action，不猜测 Flow 序列、路径、Gate 或并行策略。
