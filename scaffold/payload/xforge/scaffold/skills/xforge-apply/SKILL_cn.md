...
3. 每个工作包只能包含 `id`、`goal`、`depends_on`、`inputs`、`write_paths`、`skills`、`verify`、`done_when` 八个字段。每条 `verify` 都写成 argv 数组——`verify: [["npm", "test"], ["npx", "eslint", "src"]]`——因为 XForge 直接以 `argv[0]` 启动进程、其余项作为字面参数，从不经过 shell：命令行无法使用管道、重定向、串联或替换。单字符串形式已废弃（按空白切分并给出警告），含 shell 元字符时直接拒绝；确实需要 shell 的逻辑请写成 `write_paths` 内的脚本再调用该脚本。让每条路径只有一个明确写者，收窄会吞掉其他包路径的父级 glob；调度时另附 `change_id`、`execution_id`、`base_commit`、dependency commits、branch、worktree 与 delivery mode，不把它们写回静态计划。
