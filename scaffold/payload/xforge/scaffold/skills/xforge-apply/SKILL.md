...
3. Each package contains only `id`, `goal`, `depends_on`, `inputs`, `write_paths`, `skills`, `verify`, and `done_when`. Write each `verify` entry as an argv array — `verify: [["npm", "test"], ["npx", "eslint", "src"]]` — because XForge spawns `argv[0]` with the rest as literal arguments and never uses a shell: a command line cannot pipe, redirect, chain, or substitute. A single string is deprecated (it is split on whitespace and warns) and is rejected outright if it contains a shell metacharacter; put anything that genuinely needs a shell into a script inside `write_paths` and call the script. Give every path one writer and narrow broad parent globs. Put `change_id`, `execution_id`, commits, branch, worktree, and delivery mode in dispatch data, not the static plan.
...
7. Main Agent checks dispatch binding, commit ancestry, actual `base...head` diff, write boundary, command exits, and semantic evidence. A delivery's recorded validation commands must match the declared `verify` entries exactly, in order — a Worker that ran something else, or ran them in a different order, has not verified this package. Every successful delivery must return `done_when_evidence`, mapping each exact `done_when` criterion once to non-empty evidence, plus unchanged revision/policy/audit bindings. Only then record `<change>/evidence/agents/<package>/<execution>.yaml` and refresh State.
...
10. Run proportionate tests, lint, and build after each coherent unit. When implementation is complete, run `npx --no-install xforge check --change <id>`, request `npx --no-install xforge transition --change <id> --to verify`, and hand off to `xforge-verify`.
