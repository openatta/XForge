...
- 先运行 `xforge state --field project --field flows --field changes --field specs`，读取 Changes 路径、项目模块、Specs，以及每个 Flow 及其用于选型的 `policy`。Constitution 按 State 报告的路径去读文件：State 只携带它的版本与路径，不再携带正文。
