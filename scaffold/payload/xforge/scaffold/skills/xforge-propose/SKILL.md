...
- Once `change.yaml` exists, every later read is `xforge stage --change <id>`: it returns where the Change stands, the ready Action with its `writes` and `requiredSections`, and under `owes` every Artifact this Stage still owes with its `instruction` and `outline`, the paths of that Action's `inputs` with a digest and section list for each, the Constitution, and the diagnostics. Open the inputs you actually need and open each one once; the reply tells you which they are and which of them moved since this Stage began.
...
- Record this project's answer for a required `builtin: declared` Gate it has never answered, using `xforge verification declare` — never by editing `xforge/manifest.yaml` yourself, which the `protected-manifest` PermissionPolicy refuses anyway. The authority sits here because the refusal does: a Flow's declared Gates block this Change's **first** Transition, before any Stage that runs one. It is still the project's answer and not yours — see Execution step 6.
...
3. Create the minimum `change.yaml` from the `create-change` Action: write its `template` to its `writes` path under a new kebab-case Change id, replacing every placeholder with a project fact. The template already carries this project's default Flow and its first module; `paths` and every classification key are yours to answer. Then run `xforge state --change <id>`.
   ...
   Continue only with ready Propose Artifacts and Actions, and clear the schema diagnostics this Change's own files caused. A diagnostic you cannot clear inside this Stage's authority — a Requirement no later Artifact has anchored yet, a Rule whose enforcement this Flow does not carry — is reported, not fixed. Reaching outside the Stage to silence one is the overstep this Skill's Authority exists to prevent. An undeclared verification Gate is the exception and step 6 is where it is answered, not here: it is a project-level question this Stage does hold the authority to record, and it is recorded once, from the project's own stated answer.
...
6. Run `xforge advance --change <id>`: it runs this Stage's Gates and, if nothing refuses, takes the Transition. Read what it reports. A Gate that refuses stops the Transition and names itself — fix only Propose-stage structural issues, and never read advisory text as a passed Gate. When more than one Transition is ready it asks which, because choosing between going forward and reworking is not a default: name it with `--to`.
   ...
   `verification:<gate>:undeclared` is the one block here that is not about this Change at all. It says this Flow schedules a Gate that runs whatever the project declares, and the project has declared nothing — a question answerable before any Change existed, asked at the only moment answering it is free. It used to be raised as a notice from this Stage onward and refuse several Stages later, and a measured run carried the notice from here to Verify without acting on it, so the same fact now blocks. **Declare every Gate it names in one pass** with the `remedy.commands` argv, which arrives with the Gate substituted and `<program>` left blank, because that blank is the part only a person can fill:

   - **The command is the project's answer, not yours.** Take it from what the project says about itself — the request document, its README, its CI configuration. `--by` names the person whose answer it is.
   - **Do not adopt the CLI's suggestion because it appeared.** It reads build-system markers, not whether a command verifies anything: `pyproject.toml` gets `pytest` suggested at a project that runs `unittest`, and a test command on a repository with no tests passes the Gate while asserting nothing.
   - **When the project states no answer, stop and ask, and stop for real.** Report which Gates are unanswered and what a person has to decide. A plausible guess is the failure this block exists to prevent, and a correct guess is still a guess — the Gate would then be green having asserted whatever that command happens to assert, permanently and for every later Change.
