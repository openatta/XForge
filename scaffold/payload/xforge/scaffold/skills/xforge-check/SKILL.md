...
- When this Change declared `classification.moduleContract: true`, the Stage also cannot exit until the `contractDecisions` exit condition is satisfied, and `evidence/conditions/contractDecisions.yaml` is a third Agent-authored ledger of the same kind. It lives under `evidence/` and is still not Gate Evidence. A Change that declared no interface change owes no such ledger and is not blocked by its absence — `xforge stage` is what says which of the two this Change is; do not write the file to be safe.
...
6. Write `evidence/constitution-check.yaml` as the Artifact's `instruction` and `outline` under `owes` define it. **Two principles are not yours to answer**: the observability one and the parallel-development one are decided by the Gate from evidence it reads itself, and it records the decision and the basis in the Gate Evidence — leave them out. An entry you write for one anyway is still evaluated, so write one when you have a violation to declare, but not to restate a result a Gate already produced. For every other principle, one entry citing something machine-locatable. Answer each against what this Change actually did, and treat `not-applicable` as a claim you have to justify rather than a way past a principle you have not thought about.
7. When this Change declared `classification.moduleContract: true`, write `evidence/conditions/contractDecisions.yaml`: one entry per interface change that needs a person to decide it — typically each breaking change the `contract-delta` declares — with exactly the four field names `question`, `decision`, `decidedBy` and `decidedAt`. There are no aliases: `resolvedBy` and `approvedBy` belong to the two ledgers above and are not read here, and a misspelled key is silent, because nothing suggests a correction for this file the way it does for the findings ledger. `decidedAt` must parse as a date, and `decidedBy` must match an approver on one of this Change's receipts or one of its Git authors — the same bar, and the same provisional pass while the Change has neither. **Do not decide these yourself.** An entry naming a person is that person's authorisation; writing one for them records an authorisation nobody gave. Put the question to the user and write what they answer. A Change that declared an interface change but has no *breaking* one to decide writes `entries: []`, which is an assertion rather than an omission. A Change that declared no interface change at all owes no file: the condition does not apply to it, and writing one anyway is a turn spent asserting something nothing asked. The CLI reports a shortfall verbatim as `condition:contractDecisions:<reason>`, and reports nothing at all when the condition does not apply — `undecided-N` names the entries, `ledger-missing-expected-resolved` means the file is not there at all.
8. When this project has selected `contract-compat`, the Check Stage also runs it, and it is a declared Gate: it refuses until this project has recorded a command with `xforge verification declare --gate-name contract-compat --command '[...]' --by <person>`. That refusal reads in `blockedBy` as `gate:contract-compat:failed`, indistinguishable from a real failure — read the diagnostic code, not the block, and never hand-edit `xforge/manifest.yaml` to fix it.
