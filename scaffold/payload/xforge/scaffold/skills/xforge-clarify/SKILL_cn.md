...
- 本 Change 一旦返工退回过 Clarify 再回来，台账里**每一条**都要带 `decidedAfter: <receiptId>`，指明它站在哪一次返工之后；**这个 id 拒绝消息里直接给出，照抄即可**。它是那张「往回走」的收据（`to` 段在 `from` 段之前），**不是 `transitions.latest`**——latest 是走回来的那一段。六趟实跑都先写了 latest、都被拒、都只能自己去翻收据链才恢复。在它与那张收据对上之前，条目一律算 stale——**`decidedAt` 不再是清掉阻塞的那个字段，改时间戳没有任何作用**。这不是形式要求：时间戳由被约束的一方自己写，一次实跑就是用 `date -u` 取当前时间清掉了这条拒绝，同时在台账里声称自己「重新确认过而非重新计时」，而记录分辨不了这两者。返工后才第一次决定的条目同样要带这个字段。
