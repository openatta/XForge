...
- Run `xforge state --change <id> --field nextActions --field diagnostics --field change.governance --field change.nextArtifact`, consume only the current-revision ready Design Action, and reread every Action input.
