...
7. On `XFORGE_VERIFICATION_NOT_DECLARED`, `XFORGE_VERIFICATION_TOOLCHAIN_UNCOVERED`, or `XFORGE_VERIFICATION_GATE_MIGRATED`, **ask the user how this project runs that check and record their answer with `xforge verification declare`**, naming them in `--by`: `xforge verification declare --gate-name <gate> --command '["<program>","<arg>"]' --by <the person who answered>`, adding `--covers`, `--module` or `--working-directory` when more than one toolchain or root is in play. A toolchain the Gate deliberately does not cover is recorded rather than left unanswered: `--not-applicable <marker> --justification <text> --by <person>`. `--dry-run` shows the block first. **Do not write `manifest.verification` by hand.** This Skill may edit the Manifest for resource *selection*, which no command performs — but a declaration is not selection, and there is a command for it that fills `declaredAt` and refuses rather than producing a Manifest that will not load. A live run hand-wrote the block one indent level short and left the governance dispatcher unable to read the Manifest at all, which denied every subsequent tool call. The `nextAction` carries any command this CLI can suggest for the build-system markers it found — treat every one as a question to put to the user, never as an answer to adopt. **Do not guess, and do not adopt a suggestion silently:** XForge cannot tell whether a command verifies anything, which is exactly why a person has to say so and be named.
