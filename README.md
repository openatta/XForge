...
> **当前版本：** `@xforge/cli 0.8.1`、Protocol 2、Node.js 20 或更高。
...
上表说的是**类别不许互相冒充**，那是真的。但每一格的强度取决于机制，
而机制在默认配置下各有边界——写在这里，而不是让你自己去撞：

- **Rule 是四者里唯一自带诚实的**——它把落差算进 `coverage`，见上。
- **PermissionPolicy 由宿主执行，不由 XForge 执行。**它是投影过去的 Hook，
  而宿主设置里的 `disableAllHooks` 一行就能关掉全部 Hook——包括被治理者自己的用户设置。
  `xforge doctor` 会报出项目内可见的那一层，并明说它看不到用户层、管理层与
  服务端下发层。
- **Gate 的 Evidence 绑定的是 Artifact 与策略快照的摘要，不是代码。**
  `gitHead` 记录了但**刻意**不参与等价判断（否则一次记账提交就让每道 Gate 失效），
  所以 `unit-tests` 的 Evidence 可以穿过任意多次产品源码提交仍算「当前」。
  `xforge state` 会报 `sourceFilesChangedSince`，但没有任何东西因它阻塞。
- **Approval 收据靠审计链背书。**一张手工放置的、格式完美的收据，
  因为链里没有对应的 `approval.decided` 事件而不被计入——这一条是真的。
  但默认那条链是**无密钥的 SHA-256**，输入全部公开，且存放在被治理的 Agent
  写得了的仓库里（`xforge/.audit/` 还整个不进 Git）。它可靠地抓住手放的收据、
  截断或损坏的日志；它**挡不住同时改写链与收据的人**。
  这是 corruption-evidence，不是 tamper-evidence。
  配上 `audit.chain.hmacSecretEnv` 才变成后者，而它出厂未设。

一句话概括这四条的共同边界：**这套记录是防篡改证据链能力所及的「可验证未被改动」，
不是「可证明完整」。**哈希链证明的是**被记录下来的东西没被改**，
不是**发生过的东西都被记录了**——控制台改动、直接改库、Flow 之外的基础设施变更，
都不在其中。
...
```bash
npm install -g @xforge/cli@0.8.1
...
```bash
npm install --save-dev --save-exact @xforge/cli@0.8.1
