...
`TEST-ARCHITECTURE.md` 说明**新测试该写在哪一层、凭什么**。
...
退出码分三种：`0` 证明了、`1` 有测试在父提交上也通过（**没证明**）、`2` 父提交构建不起来（**没跑成**）、
`3` 这次改动没加测试（没什么要证的）。`verify` 用 `--allow-nothing-to-prove` 把 `3` 当成 0，
`1` 和 `2` 照常失败——这两件事以前是同一个退出码，正因如此它一直没法接进闸门。

**「因为兼容性而改的测试文件」也是候选，这一条踩过。** 给一个类型加了必填字段之后，
三处手写 fixture 必须补字段；那个文件因此进入候选、在父提交上照样通过、判 `NOT PROVED`，
整个 verify 红。两条出路：给那个文件补一个真正区分新行为的断言（推荐——那个行为本来就该有测试），
或者让新字段可选从而不必改 fixture（只有在它确实可选时才诚实）。
**推论：纯粹的测试文件移动做不到**，换目录而内容不变的文件必然 `NOT PROVED`。
...
（当时阈值 78%），而代码的实际执行一点没少。

**两条路径的差别不只是测量。** 进程内跑的是 vitest worker，spawn 跑的是 `dist/cli.js`——
`xforge version` 的 `executablePath` 因此在两边给出不同的答案，而这是**产品的真实行为**
...
**要掉十个点闸门才响**——足够让一整个没测试的模块进来。实际值涨了就把阈值跟着抬，
不抬的阈值等于没人在守。
