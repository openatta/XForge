...
/*
 * `readdirSync` was used at two call sites and imported at none. Both sit inside a `try`, so the
 * ReferenceError was caught and each silently took its failure branch on every run that reached it:
 * a glob Artifact always read as not produced, and an Approval policy always read as holding zero
 * receipts. A crash would have been the kinder outcome -- these answered wrongly and confidently.
 */
