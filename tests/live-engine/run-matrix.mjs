...
let policy = createLiveEnginePolicy({
  ...
  stages: [...stages.map((stage) => stage.id), ...(scenarioConfig.inject ? [scenarioConfig.inject.stageLabel] : []), 'archive'],
});
...
for (let index = 0; index < stages.length; index += 1) {
  ...
  if (scenarioConfig.inject?.afterStage === stage.id) {
  ...
  if (stage.exit?.approvals?.length) {
    ...
    const state = runXforgeJson(projectRoot, ['state', '--change', scenarioConfig.changeId]);
    if (state.data.change.governance.currentStage !== nextStage.id) {
      throw new Error(`Agent did not self-transition ${stage.id} -> ${nextStage.id} as instructed (currentStage=${state.data.change.governance.currentStage}).`);
    }
  }

  if (nextStage?.execution && nextStage.execution.workPackages !== 'internal') {
