...
Skill. Use `npx --no-install xforge` from the project root and follow its
...
targets. Run `npx --no-install xforge sync --dry-run` and then
`npx --no-install xforge sync` to confirm it is picked up. Do not create a
