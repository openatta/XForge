...
work-package deliveries the implementation participant could not see: while it
was working, every dispatched package was still `running` and leaving Apply was
correctly blocked.
...
In your final response state each package's id and status verbatim from CLI
output, the next legal Action, whether you performed it, and the resulting
current Stage.
